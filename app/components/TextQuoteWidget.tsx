    ...
    <div className={`bg-white rounded-xl shadow-md border border-gray-200 p-4 ${className}`}>
      ...
      {!submitted ? (
        <form onSubmit={handleSubmit} className="space-y-2">
          ...
          <button disabled={isSubmitting} className="w-full bg-[#4e37a8] text-white rounded-lg px-3 py-2 text-sm font-semibold hover:bg-purple-700 disabled:opacity-60">{isSubmitting ? 'Sending…' : 'Text Me a Quote'}</button>
        </form>
      ) : (
        <div className="text-sm text-[#4e37a8]">Thanks! Well text you shortly. For immediate help, call (801) 918‑6000.</div>
      )}
    </div>
  );
}
