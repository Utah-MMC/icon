...
export default function SellHouseForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch("/api/chat", {
        ...
        body: JSON.stringify({ action: "lead", ...payload, leadType: "sell_house", pageUrl: "/sell-house" }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <button type="submit" disabled={isSubmitting} className="col-span-1 sm:col-span-2 inline-flex items-center justify-center bg-[#4e37a8] text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition-colors font-semibold">
