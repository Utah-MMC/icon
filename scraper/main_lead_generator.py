#!/usr/bin/env python3
"""
Icon Dumpsters - Advanced Lead Generation System
Main Orchestrator

This system provides a comprehensive lead generation solution that:
1. Scrapes multiple sources for potential dumpster rental leads
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('icon_dumpsters_leads.log'),
...
class IconDumpstersLeadGenerator:
    def __init__(self, config_file: str = 'config.json'):
        self.config_file = config_file
        self.config = self.load_config()
        
        # Initialize all systems
        self.lead_generator = EnhancedLeadGenerator(self.config.get('database_path', 'dumpster_leads.db'))
        self.nurturing_system = LeadNurturingSystem(
            self.config.get('database_path', 'dumpster_leads.db'),
            self.config.get('email_config')
        )
        self.analytics_system = LeadAnalyticsSystem(self.config.get('database_path', 'dumpster_leads.db'))
        
        logger.info("Icon Dumpsters Lead Generation System initialized")
    
    def load_config(self) -> Dict:
        """Load configuration from JSON file"""
        default_config = {
            'database_path': 'dumpster_leads.db',
            'email_config': {
            ...
            'lead_scoring': {
                'high_value_keywords': ['dumpster rental', 'roll off dumpster', 'construction dumpster', 'demolition dumpster', 'junk removal', 'contractor cleanup', 'construction cleanup'],
                'medium_value_keywords': ['dumpster', 'waste removal', 'debris removal', 'construction cleanup', 'project cleanup', 'site cleanup', 'building cleanup'],
    ...
    def get_quick_stats(self) -> Dict:
        ...
        try:
            import sqlite3
            conn = sqlite3.connect(self.config.get('database_path', 'dumpster_leads.db'))
            cursor = conn.cursor()
    ...
    def export_leads(self, filename: str = None) -> str:
        ...
        if not filename:
            filename = f"icon_dumpsters_leads_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    parser = argparse.ArgumentParser(description='Icon Dumpsters Lead Generation System')
    ...
    generator = IconDumpstersLeadGenerator(args.config)
    ...
    try:
        ...
        if args.mode != 'stats' and args.mode != 'export':
            print(f"\nDetailed results saved to log file: icon_dumpsters_leads.log")
