...
function updateCityPage(cityName) {
  ...
  if (content.includes('historicalInfo')) {
    console.log(`Historical data already exists for ${cityName}`);
    return false;
  }
  
  // Find the CityPageClient component call
  const cityPageClientMatch = content.match(/<CityPageClient\s+([^>]+)\s*\/>/);
  if (!cityPageClientMatch) {
    console.log(`Could not find CityPageClient component in ${cityName}`);
    return false;
  }
  
  // Create the new props string with historical data
  const newProps = `cityName="${cityName}"
      cityDisplayName="${cityData.displayName}"
      nearbyCities={nearbyCities}
      historicalInfo="${cityData.historicalInfo.replace(/"/g, '\\"')}"
      landmarks="${cityData.landmarks.replace(/"/g, '\\"')}"
      localProjects="${cityData.localProjects.replace(/"/g, '\\"')}"
      neighborhoods="${cityData.neighborhoods.replace(/"/g, '\\"')}"
      localBusinesses="${cityData.localBusinesses.replace(/"/g, '\\"')}"`;
  
  // Replace the CityPageClient component call
  ...
  console.log(`Added historical data to ${cityName}`);
  return true;
}

// Process all city pages
console.log('Adding historical data to city pages...');
let updatedCount = 0;
...
console.log(`\nCompleted! Added historical data to ${updatedCount} city pages.`);
